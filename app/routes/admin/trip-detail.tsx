...
import { getAllTrips, getTripId } from "~/appwrite/trips";
...
export const loader = async ({ params }: LoaderFunctionArgs) => {
   ...
   // Multiple async process that don't depend on one another, use promise all
   const [trip, trips] = await Promise.all([
      await getTripId(tripId),
      await getAllTrips(4, 0),
   ]);
   // const trip = await getTripId(tripId); //500ms
   // const trips = await getAllTrips(4, 0); //500ms
